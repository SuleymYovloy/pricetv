...
import {
    getProgrammListFetching,
    getProgrammListSuccess,
    getProgrammListErorr,
    getShift,
} from "../reducers/getProgrammListSlice";
...
export const getProgrammList =
    (country: string, page: number, program: string, shift: number) =>
    async (dispatch: AppDispatch) => {
        try {
            ...
            const { data } = await axios.get(
                `${url}/api/v1/product-list/?country=${country}&page=${page}&page_size=30&program=${program}`,
                {
                    headers: {
...
export const nextPageProgrammList =
    (nextPage: string, shift: number) => async (dispatch: AppDispatch) => {
        try {
            dispatch(getProgrammListFetching());
